<script lang="ts">
	...
	async function handlePasskeyLogin(passkey: string) {
	  try {
		await auth.loginWithPasskey(passkey);
		goto('/');
	  } catch (error) {
  ...
  <div class="login-container">
	<div class="login-box">
	  ...
	  <div class="social-login">
		...
		<button on:click={() => handlePasskeyLogin('passkey')} class="social-login-button passkey-login" aria-label="Passkey로 로그인">
		  <svg viewBox="0 0 24 24" class="social-icon">
